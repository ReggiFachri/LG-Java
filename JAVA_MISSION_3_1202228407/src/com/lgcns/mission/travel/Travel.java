...
public class Travel {
	...
	public void printTravelInfo() {
		...
		// System.out.println(
		// this.travelCode+"\t"+this.cityName+"\t"+this.flight+"\t"+type+"\t"+this.maxPeople+"\t"+this.resvPeople+"\t"+status);

		String format = "%-15s\t%-15s\t%-20s\t%-15s\t%-20s\t%-25s\t%-15s\n";
		System.out.printf(format, this.travelCode, this.cityName, this.flight, type, this.maxPeople, this.resvPeople,
				status);
	}
